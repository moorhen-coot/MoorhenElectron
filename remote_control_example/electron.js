...
function createWindow() {
  ...
  if(process.argv.length>2){
      ...
      if(!isDev) {
          ...
          exp.get('/getCommandStack', (req, res) => {
              res.send(commandStack);
              commandStack = [];
          });

          exp.get('/load/protocal/:protocol/server/:server/path/:path([/.a-zA-Z0-9-]+$)', (req, res) => {
              res.send(req.params);
              const fname = req.params.path.split("/")[req.params.path.split("/").length-1];
              const name = fname.replace(/\.[^/.]+$/, "");
              commandStack.push({"load":req.params.protocol+"://"+req.params.server+"/"+req.params.path,name:name});
          });
